# Simple example to read creds file, connect to API, and add hashes to globalqurantine from a csv.
...
df = pd.read_csv("GlobalQuarantine_20201203.csv")

for index, row in df.iterrows():

    #print content of each row in column "SHA256 HASH"
    print(row["SHA256 Hash"])

    #edit comment as needed
    #edit column name according to csv
    API.add_to_global_list("quarantine", "import from old tenant", row["SHA256 Hash"], category="None")
